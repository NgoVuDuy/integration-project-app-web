package com.shop.shopnow.activities;

import androidx.annotation.NonNull;
import androidx.appcompat.app.AppCompatActivity;

import android.content.Intent;
import android.os.Bundle;
import android.view.Menu;
import android.view.MenuItem;
import android.widget.TextView;

import com.shop.shopnow.R;

public class News2Activity extends AppCompatActivity {

    String tieuDe, noiDung;
    TextView tieuDeTV, noiDungTV;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_news2);

        Intent intent = getIntent();

        tieuDe = intent.getStringExtra("tieude");
        noiDung = intent.getStringExtra("noidung");

        //Ánh xạ view
        tieuDeTV = findViewById(R.id.titleTV);
        noiDungTV = findViewById(R.id.noiDungTV);
        //Set tiêu đề và nội dung cho text view
        tieuDeTV.setText(tieuDe);
        noiDungTV.setText(noiDung);

    }
    @Override
    public boolean onCreateOptionsMenu(Menu menu) {
        getMenuInflater().inflate(R.menu.menu_back, menu);
        return super.onCreateOptionsMenu(menu);
    }

    // Xét sự kiện ấn vào menu
    @Override
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {

        if(item.getItemId() == R.id.back_item) {
            onBackPressed();
        }
        return super.onOptionsItemSelected(item);
    }
}